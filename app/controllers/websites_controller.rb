...
require 'nokogiri'
  ...
  def fayette_mc
  doc = Nokogiri::HTML(open('http://www.faycom.info/showcase.php?action=list&range=future'))
  @test = Array.new
  doc.search('.style1 > font > font').each do |text|
    unless text.content =~ /^Sale/
      @test << text.content
    end
  end
  @test.each do |street|
    address = Address.new do |a|
      a.street_line_1 = street
      a.city = 'Lexington'
      a.state = 'Ky'
    end
    address.save
